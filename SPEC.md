...
- [x] **cli string unicode** — verifies: PKL-122 — tags: moonbit, cli, stdlib, string, unicode, contract
  > The native CLI evaluates a fixture that observes a String whose last glyph is a supplementary-plane code point (`🍣`, U+1F363). Apple Pkl's `String.length` is the code-point count (the upstream `api/stringUnicode` fixture asserts `"😀😈😍😎😡🤢🤣".length == 7`), so `length` and `codePointCount` both report `4` for `"abc🍣"`. `codePoints` and `chars` walk the code-point stream, and `codePointAt(3)` returns the supplementary code point as an Int.
  - body: `cmd` (exit 0 expected)
