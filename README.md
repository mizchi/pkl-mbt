...
As of the `PKL-092` spec slice, this project has 96 implemented pkspec scenarios and a 2-entry roadmap of draft slices in `specs/Roadmap.pkl`. The next tracked slice is `PKL-089` (generic class declarations); `PKL-090` (generic function type parameters) follows once `PKL-089` lands. Both are major slices that require type-parameter propagation through the typechecker.
