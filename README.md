...
As of the `PKL-084` spec slice, this project has 94 implemented pkspec scenarios and a 4-entry roadmap of draft slices in `specs/Roadmap.pkl`. PKL-084 originally bundled `read` with `throw` / `trace`, but `read` was split off as the new `PKL-098` (`read` built-in with explicit sandbox policy) since it forces an external-state policy decision distinct from the pure value-side builtins. The next tracked slice is `PKL-098`. The remaining roadmap entries (`PKL-089`/`PKL-090` generic class & function, `PKL-092` Float constraint predicates) are larger and gated on architectural decisions or prerequisite Float numerics.
