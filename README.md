...
As of the `PKL-076` spec slice, this project has 75 implemented pkspec scenarios and a 22-entry roadmap of draft slices in `specs/Roadmap.pkl`. The next tracked slice is `PKL-077`, which extends the stdlib pivot to `pkl:base` String operations (toUpperCase / toLowerCase / split / replaceAll / startsWith / endsWith / indexOf / etc.). The constraint round-off task `PKL-069` is kept in the roadmap as a draft.
...
| Stdlib & rendering | 20-25% | `pkl:math.maxInt32`, the `pkl:test.catch` flow, the core `pkl:base` Listing surface (length/isEmpty/first/last/distinct/contains/reverse/take/drop/join/map/filter/fold), and the core `pkl:base` Mapping surface (length/isEmpty/keys/values/containsKey/getOrNull/fold) are wired on the stdlib side. The PCF renderer is byte-for-byte against upstream gold for the implemented language slice, `eval -f json` emits Apple Pkl's `pkl eval -f json` shape, `eval -f yaml` emits Apple Pkl's `pkl eval -f yaml` block-style shape (modulo block scalars for multiline strings, which still emit as double-quoted), and `eval -f properties` emits the Java Properties shape (dotted flatten, JSON-encoded listings, null leaves omitted). Plist renderer, generators, String / Int / Float / Set stdlib operations, `pkl:reflect`, regex / duration / bytes, and the rest of the Apple Pkl stdlib surface are unimplemented. This is still the largest remaining gap. |
...
This is not a full Pkl implementation yet. With the four-renderer surface (PCF / JSON / YAML / Properties) and the core `pkl:base` Listing / Mapping operations in place, the next compatibility work pivots to `PKL-077` `pkl:base` String operations (toUpperCase, toLowerCase, split, replaceAll, startsWith, endsWith, etc.); the full draft roadmap lives in `specs/Roadmap.pkl`.
