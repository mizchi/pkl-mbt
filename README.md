...
As of the `PKL-080` spec slice, this project has 93 implemented pkspec scenarios and a 4-entry roadmap of draft slices in `specs/Roadmap.pkl`. The next tracked slice is `PKL-084` (built-in `read` / `trace` / `throw`), gated on the sandbox-policy decision for external state. The remaining roadmap entries (`PKL-089`/`PKL-090` generic class & function, `PKL-092` Float constraint predicates) are larger and gated on architectural decisions or prerequisite Float numerics.
