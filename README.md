...
- **Eval gold-match**: 301 / 391 PCF fixtures byte-for-byte against upstream (77.0%)
- **Renderer gold-match**: JSON 1, plist 1, textproto 1, XML 11 upstream renderer fixtures byte-for-byte against upstream
...
- `basic`: 79 / 86 with-gold fixtures pass. Numeric / Bytes parity is gold-matching; remaining DIFFs cluster around `as`, `new`, nullable, amend-chain, underscore, and const / `outer` provenance.
