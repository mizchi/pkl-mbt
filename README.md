...
As of the `PKL-078` spec slice, this project has 77 implemented pkspec scenarios and a 20-entry roadmap of draft slices in `specs/Roadmap.pkl`. The next tracked slice is `PKL-079`, which expands `pkl:math` beyond `maxInt32` (min, max, abs, sqrt, pow, log, exp, ceil, floor, round and the matching constants). The constraint round-off task `PKL-069` is kept in the roadmap as a draft.
...
| Stdlib & rendering | 25-30% | `pkl:math.maxInt32`, the `pkl:test.catch` flow, the core `pkl:base` Listing surface, the core `pkl:base` Mapping surface, the core `pkl:base` String surface, and the core `pkl:base` Int surface (abs/isEven/isOdd/toString/toString(radix)/toChar) are wired on the stdlib side. The PCF renderer is byte-for-byte against upstream gold for the implemented language slice, `eval -f json` / `eval -f yaml` / `eval -f properties` all match Apple Pkl's projection rules (modulo YAML block scalars for multiline strings). Plist renderer, generators, Float numerics, `pkl:math` expansion (sqrt/pow/log/etc.), `pkl:reflect`, regex / duration / bytes / Set, and the rest of the Apple Pkl stdlib surface are unimplemented. This is still the largest remaining gap. |
...
This is not a full Pkl implementation yet. With the four-renderer surface (PCF / JSON / YAML / Properties) and the core `pkl:base` Listing / Mapping / String / Int builtins in place, the next compatibility work pivots to `PKL-079` `pkl:math` expansion (min, max, abs, sqrt, pow, log, exp, ceil, floor, round and the matching constants); the full draft roadmap lives in `specs/Roadmap.pkl`.
