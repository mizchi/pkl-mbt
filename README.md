...
- Generic class declarations such as `class Box<T> { value: T }` and `class Pair<A, B> { first: A; second: B }`. The parser captures the `<T1, T2, ...>` list on `ClassDecl.type_parameters`, and the typechecker injects each parameter into a class-scoped `type_env` as a binding to `UnknownType` so body uses of `T` route through the existing tolerated-annotation path instead of failing
- Generic function declarations such as `function identity<T>(x: T): T = x` and `function pair_first<A, B>(a: A, b: B): A = a`. The parser captures the `<T1, T2, ...>` list on `FunctionDecl.type_parameters`, the module-level `collect_declared_types_with_imports` flattens parameters into the shared `type_env` as `UnknownType`, and the runtime callable-argument / callable-return validators short-circuit on type parameter names (accept-any) via `eval_type_name_is_type_parameter`. Call-site inference of T from the argument type stays deferred
...
As of the `PKL-090` spec slice, this project has 98 implemented pkspec scenarios and an empty roadmap of draft slices in `specs/Roadmap.pkl`. The original 25-slice forward-looking roadmap has now landed end-to-end. Future work focuses on inference upgrades (call-site T-substitution, real `TypeVariable` propagation), the `read?(uri)` parser form, and the deferred `output { renderer }` driver path — those will be filed as fresh roadmap entries when they get scheduled.
