...
As of the `PKL-073` spec slice, this project has 72 implemented pkspec scenarios and a 25-entry roadmap of draft slices in `specs/Roadmap.pkl`. The next tracked slice is `PKL-074`, which adds a Java Properties renderer matching Apple Pkl's `pkl eval -f properties` shape. The constraint round-off task `PKL-069` is kept in the roadmap as a draft.
...
| Stdlib & rendering | 12-17% | Only `pkl:math.maxInt32` and the `pkl:test.catch` flow are wired on the stdlib side. The PCF renderer is byte-for-byte against upstream gold for the implemented language slice, `eval -f json` emits Apple Pkl's `pkl eval -f json` shape, and `eval -f yaml` emits Apple Pkl's `pkl eval -f yaml` block-style shape (modulo block scalars for multiline strings, which still emit as double-quoted). Properties / plist renderers, generators, `pkl:base` extensions, `pkl:reflect`, regex / duration / bytes, and the rest of the Apple Pkl stdlib surface are unimplemented. This is still the largest remaining gap. |
...
This is not a full Pkl implementation yet. The next compatibility work continues the renderer pivot with `PKL-074` Properties rendering; the full draft roadmap lives in `specs/Roadmap.pkl`.
