...
- **Eval gold-match**: 281 / 391 PCF fixtures byte-for-byte against upstream (71.9%)
...
- `basic`: 70 / 86 with-gold fixtures pass. Remaining DIFFs cluster around `as` edge cases, numeric and Bytes parity, resource/glob host behavior, and const / `outer` provenance.
- `generators`: 20 / 21 with-gold fixtures pass. Element / entry generators, typed generator binding checks, function-body and mixin generators, predicate members, and Dynamic / Listing / Mapping spread syntax are done; only the lexical-scope generator case remains.
