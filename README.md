...
As of the `PKL-098` spec slice, this project has 95 implemented pkspec scenarios and a 3-entry roadmap of draft slices in `specs/Roadmap.pkl`. The next tracked slice is `PKL-092` (Float constraint predicates). The remaining roadmap entries (`PKL-089`/`PKL-090` generic class & function) are larger and gated on architectural decisions for type-parameter propagation through the typechecker.
