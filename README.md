...
As of the `PKL-077` spec slice, this project has 76 implemented pkspec scenarios and a 21-entry roadmap of draft slices in `specs/Roadmap.pkl`. The next tracked slice is `PKL-078`, which extends the stdlib pivot to `pkl:base` Int / Float numeric operations (abs, isEven, isOdd, toFloat, toString, toString(radix), toChar). The constraint round-off task `PKL-069` is kept in the roadmap as a draft.
...
| Interpreter | 45-55% | Arithmetic, objects, imports, module amends/extends, collections, class defaults/inheritance, method calls, direct function/lambda calls, callable runtime values, function/lambda/method return validation including built-in and user-defined constrained return predicates, callable and method argument validation including built-in and user-defined constrained predicates, typealias resolution for callable return annotations, lexical closure captures for scalar/object/callable values, simple constrained value annotations, constrained callable arguments, constrained method arguments, constrained typealias values/members, selected numeric constraints, multiple constraint lists, negated constraints, plain-function user-defined numeric constraint factories, constrained class property values/defaults, the upstream `classes/constraints8.pkl` catch flow, the `pkl:base` Listing operations (length/isEmpty/first/last/distinct/contains/reverse/take/drop/join/map/filter/fold), the `pkl:base` Mapping operations (length/isEmpty/keys/values/containsKey/getOrNull/fold), and the `pkl:base` String operations (length/isEmpty/toUpperCase/toLowerCase/contains/startsWith/endsWith/indexOf/replaceAll/replaceFirst/take/drop/split/padStart/padEnd) work. Generators, non-numeric constraints (Float / Boolean / collection element), and many external functions are still incomplete. |
...
| Stdlib & rendering | 23-28% | `pkl:math.maxInt32`, the `pkl:test.catch` flow, the core `pkl:base` Listing surface, the core `pkl:base` Mapping surface, and the core `pkl:base` String surface (length/isEmpty/toUpperCase/toLowerCase/contains/startsWith/endsWith/indexOf/replaceAll/replaceFirst/take/drop/split/padStart/padEnd) are wired on the stdlib side. The PCF renderer is byte-for-byte against upstream gold for the implemented language slice, `eval -f json` emits Apple Pkl's `pkl eval -f json` shape, `eval -f yaml` emits Apple Pkl's `pkl eval -f yaml` block-style shape (modulo block scalars for multiline strings, which still emit as double-quoted), and `eval -f properties` emits the Java Properties shape (dotted flatten, JSON-encoded listings, null leaves omitted). Plist renderer, generators, Int / Float / Set stdlib operations, `pkl:reflect`, regex / duration / bytes, and the rest of the Apple Pkl stdlib surface are unimplemented. This is still the largest remaining gap. |
...
This is not a full Pkl implementation yet. With the four-renderer surface (PCF / JSON / YAML / Properties) and the core `pkl:base` Listing / Mapping / String operations in place, the next compatibility work pivots to `PKL-078` `pkl:base` numeric operations (abs, isEven, isOdd, toFloat, toString, toString(radix), toChar); the full draft roadmap lives in `specs/Roadmap.pkl`.
