...
As of the `PKL-075` spec slice, this project has 74 implemented pkspec scenarios and a 23-entry roadmap of draft slices in `specs/Roadmap.pkl`. The next tracked slice is `PKL-076`, which extends the stdlib pivot to `pkl:base` Mapping operations (containsKey, keys, values, fold, getOrNull, entries). The constraint round-off task `PKL-069` is kept in the roadmap as a draft.
...
| Interpreter | 42-52% | Arithmetic, objects, imports, module amends/extends, collections, class defaults/inheritance, method calls, direct function/lambda calls, callable runtime values, function/lambda/method return validation including built-in and user-defined constrained return predicates, callable and method argument validation including built-in and user-defined constrained predicates, typealias resolution for callable return annotations, lexical closure captures for scalar/object/callable values, simple constrained value annotations, constrained callable arguments, constrained method arguments, constrained typealias values/members, selected numeric constraints, multiple constraint lists, negated constraints, plain-function user-defined numeric constraint factories, constrained class property values/defaults, the upstream `classes/constraints8.pkl` catch flow, and the `pkl:base` Listing operations (length/isEmpty/first/last/distinct/contains/reverse/take/drop/join/map/filter/fold) work. Broad stdlib behavior beyond Listing, generators, non-numeric constraints (String / Float / Boolean / collection element), and many external functions are still incomplete. |
...
| Stdlib & rendering | 18-23% | `pkl:math.maxInt32`, the `pkl:test.catch` flow, and the core `pkl:base` Listing surface (length/isEmpty/first/last/distinct/contains/reverse/take/drop/join/map/filter/fold) are wired on the stdlib side. The PCF renderer is byte-for-byte against upstream gold for the implemented language slice, `eval -f json` emits Apple Pkl's `pkl eval -f json` shape, `eval -f yaml` emits Apple Pkl's `pkl eval -f yaml` block-style shape (modulo block scalars for multiline strings, which still emit as double-quoted), and `eval -f properties` emits the Java Properties shape (dotted flatten, JSON-encoded listings, null leaves omitted). Plist renderer, generators, Mapping / String / Int / Float / Set stdlib operations, `pkl:reflect`, regex / duration / bytes, and the rest of the Apple Pkl stdlib surface are unimplemented. This is still the largest remaining gap. |
...
This is not a full Pkl implementation yet. With the four-renderer surface (PCF / JSON / YAML / Properties) and the core `pkl:base` Listing operations in place, the next compatibility work pivots to `PKL-076` `pkl:base` Mapping operations (containsKey, keys, values, fold, getOrNull, entries); the full draft roadmap lives in `specs/Roadmap.pkl`.
